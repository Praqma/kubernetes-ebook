...
Even if you use a dedicated node for each component there would still be no high availability to safeguard against the failure of "non-worker" components of the cluster, such as the etcd node and the controller node. And even if the pods survive a failing node, the cluster itself will not survive if either the controller node or the etcd node fails. So, we need to have high availability for these components too. 
...
Though not absolutely necessary we can also use a shared storage solution to demonstrate the concept of network mounted volumes (todo - ????) . We *can* use the Load balancer nodes to setup NFS and make it Highly available.

**Note about NodePort:**
In Kubernetes, `NodePort` is the best way to expose a service to the outside world when you specify it in the `kubectl expose <service> --type=NodePort` (todo - confirm / verify). This allows you to expose a service on the worker node, just like you can do with the port mapping on a simple Docker host. This makes the service available through all the worker nodes because node port works its magic by setting up certain iptables rules on all of them. The service will respond to you no matter which port you land on (from outside the cluster). However, to achieve this, you need to have the external DNS pointing to the worker nodes. e.g. www.example.com can have two A type addresses in DNS pointing to the IP addresses of two worker nodes. If a node fails DNS will do its own DNS round robin. Sometimes this works but DNS round robin generally doesn't work well during sticky sessions. Also, managing services through ports can be a difficult system to maintain. And it's not very cool either! 


## The choice of HA technology for etcd, controllers and load balancers:

We selected three nodes for the etcd service. Firstly, because it stores the metadata about our cluster - which is very important. Secondly, etcd nodes can form their own independent cluster and to satisfy the needs of a quorum we decided to use three nodes for it. This means that failure of one node does not affect the etcd operation. (todo: rewrite:) In order for the etcd cluster to be healthy it needs to have quorum. i.e. available nodes that are more than half of the size of the total number of nodes of the etcd cluter. So, having a total of only two nodes is not desirable. When one fails the etcd custer will instantly become unhealthy. This is the last thing we want to happen in a production cluster.

We know that all Kubernetes components watch the API server, running on controller node. So we have to make sure that the control plane remains available at all times. Unfortunately, controller nodes are not cluster-aware, so we need to provide some sort of HA for them. This high availability can be provided in two ways: 

* By setting up a proxy/load balancer (using HAProxy on our load balancer). This would mean that the load balancer has to be the first component to come alive when the cluster boots up, so the cluster will be dependent on the load balancer nodes. This is easier to setup but has a dependency problem.
* By setting up IPVS/LVS (using heartbeat, etc) on the controller nodes themselves and only having a floating IP/VIP on the two nodes. This way we do not have to wait for the load balancer to come up before the rest of the cluster. We then use this *Controller VIP* in the worker nodes to point to the controller node. This involves installing and configuring some HA software on controller nodes, but it doesn't have dependency problems.

We believe the second method holds more value, so we'll opt for that one. But for the sake of completeness we will also provide a method using the HAProxy.
...
So now we know all about HA! Let's intoduce some talk about Kubernetes networking as well!
...
* Pod Network (Overlay or CNI) (We are using CNI network)
* Service Network (aka. Cluster Network)

The first two (Infrastructure and Pod networks) are relatively straightforward. The *Infrastructure network* is where the actual physical (or virtual) machines are connected. The *Pod network* can be either an overlay network or a CNI based network. Normally, overlay networks are software defined networks (SDN) - Flannel is a good example of an overlay network. The overlay network is spread over several cluster nodes, and each node will have a subnet of the overlay network configured within it. Ordinarily, only the worker nodes will have the overlay network configured. To achieve this a special service such as flannel will have to run on the nodes. Then you have to configure Docker to use the network to create containers. As we already observed the overlay network is only configured on worker nodes, and it is only accessible within the worker nodes. If you want to access pod IPs (belonging to this overlay network) from other cluster nodes you will need to run an overlay network service like flannel. Only then will the pods become accessible from that node.

In CNI networking we find another relatively large overlay network. Each worker node obtains a subnet from the big (sort of overlay) network and configures a bridge (cbr0) using that subnet. Each worker node creates pods/containers in its own subnet. These subnets *do not* talk to each other. To make them talk to each other we add the routes of each subnet to the router (working as default gateway) connected to our cluster network. Now a request from any node in the cluster (or even from outside) can consult the routing table and reach the required pods. There is no need to run an additional service and the pod IPs, regardless of which worker node the pods are on, are accessible from any node within the cluster. This is the network shown in the diagram at the beginning of this document. 


The third (Service network) is not actually a network. Well, it is and it isn't - it's complicated! It is used when you decide to expose a Kubernetes deployment as a service. That is the only time it comes into play. When you expose a deployment as a service Kubernetes assigns it a Cluster IP from the *service* IP range. This IP is then used by kube-proxy running on each worker node to write special IPTables rules so that these services are accessible from pods running anywhere on the worker nodes. The Service IP, or Cluster IP, is actually just an abstraction and represents the pods at the backend where they actually belong to a *Deployment* (formerly known as *Replication Controller*).

## How does networking look in Kubernetes?

So, when you setup a Kubernetes cluster and start up your first pod it is deployed on a worker node. The pod gets an IP from the pod network which can be a type of overlay network or a type of CNI based network. Either way, when the pod gets an IP from the pod network the worker nodes are the only *nodes* which can access that pod. Of course, if you run more pods they can access each other directly. To access these nodes from outside the cluster you still need to do a bit of work. We know that every pod is actually part of a *Deployment* object (formerly *RC* or *Replication Controller*), so you expose this deployment as a service which then automatically assigns it a *Cluster IP*. At this stage the Cluster IP is still not accessible from outside the cluster. This new *service* can now be referenced/accessed by the pods using a special name formatted as a DNS name. While exposing the deployment as a service you also assign it an *External IP* which normally belongs to the infrastructure network. Although this external IP still does no good directly. It is barely used as a label. If you try to access this external IP from anywhere in the cluster, or from anywhere on the infrastructure, it will not be accessible. 
